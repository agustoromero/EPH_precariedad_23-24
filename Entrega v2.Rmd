...
Como este procesamiento fue pensado para construir series y cuadros que puedan replicarse para todas las bases de interés, definidas con anterioridad, construimos la gran mayoria de cuadros y tablas a partir de funciones, como se presenta en el siguiente chunk.
...
# Agregar la variable ANO4 a la base antes de agrupar
C._preca.intensi_sexo_part_ano <- base_asalariados %>%
  group_by(ANO4, descuento_jubil) %>%
  summarise(
    Poblacion_Ambos = sum(PONDERA),
    Poblacion_Varones = sum(PONDERA[CH04 == 1]),
    Poblacion_Mujeres = sum(PONDERA[CH04 == 2]),
    .groups = "drop"
  ) %>%
  mutate(
    'Prop. varones' = Poblacion_Varones / Poblacion_Ambos,
    'Prop. mujeres' = Poblacion_Mujeres / Poblacion_Ambos,
    'Part. Ambos (%)' = 100 * Poblacion_Ambos / sum(Poblacion_Ambos),
    'Part. Varones (%)' = 100 * Poblacion_Varones / sum(Poblacion_Varones),
    'Part. Mujeres (%)' = 100 * Poblacion_Mujeres / sum(Poblacion_Mujeres)
  )

# Separar los datos por año
data_ano1 <- C._preca.intensi_sexo_part_ano %>% filter(ANO4 == 2023) %>% select(-ANO4)
data_ano2 <- C._preca.intensi_sexo_part_ano %>% filter(ANO4 == 2024) %>% select(-ANO4)

# Unir los datos por total_precario
c.comparacion_preca_sexo <- left_join(data_ano1, data_ano2, by = "total_precario", suffix = c("_2023", "_2024"))

# Calcular diferencias por fila (2024 - 2023)
c.comparacion_preca_sexo<- c.comparacion_preca_sexo %>%
  mutate(across(starts_with("Poblacion_") | starts_with("Prop.") | starts_with("Part."),
                ~ .[[2]] - .[[1]], 
                .names = "Diferencia_{.col}"))

# Ver resultado
print(c.comparacion_preca_sexo)
```

```{r message=TRUE, warning=FALSE, paged.print=TRUE}
#1 2023
C.prueba_sexo_preca <- base_asalariados %>% filter(ANO4==2023 )%>% group_by(descuento_jubil) %>% summarise(
      Poblacion_Ambos = sum(PONDERA),
      Poblacion_Varones = sum(PONDERA[CH04 == 1]),
      Poblacion_Mujeres = sum(PONDERA[CH04 == 2]),
      .groups = "drop",
...
C._preca.intensi_sexo_part <- C.prueba_sexo_preca %>%
  mutate(
    'Part. Ambos (%)' = 100 * Poblacion_Ambos / sum(Poblacion_Ambos),
    'Part. Varones (%)' = 100 * Poblacion_Varones / sum(Poblacion_Varones),
    'Part. Mujeres (%)' = 100 * Poblacion_Mujeres / sum(Poblacion_Mujeres)
  ) %>% select(-c(Poblacion_Ambos,Poblacion_Varones,Poblacion_Mujeres,`Prop. mujeres`,`Prop. varones`))
print(C._preca.intensi_sexo_part)
...
```{#2}
C._preca.intensi_sexo <- base_asalariados %>%
  group_by(total_precario) %>%
  summarise(
    Poblacion_Ambos = sum(PONDERA),
    Poblacion_Varones = sum(PONDERA[CH04 == 1]),
    Poblacion_Mujeres = sum(PONDERA[CH04 == 2]),
...
# **Cuadro con participación (%) por columna**
C._preca.intensi_sexo_part <- C._preca.intensi_sexo %>%
  mutate(
    'Part. Ambos (%)' = 100 * Poblacion_Ambos / sum(Poblacion_Ambos),
    'Part. Varones (%)' = 100 * Poblacion_Varones / sum(Poblacion_Varones),
    'Part. Mujeres (%)' = 100 * Poblacion_Mujeres / sum(Poblacion_Mujeres)
